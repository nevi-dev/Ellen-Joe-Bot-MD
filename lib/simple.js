...
reply: {
            ...
            async value(jid, text = '', quoted, options = {}) {
                ...
                const newsletterJid = global.newsletterJid || '120363418071540900@newsletter'
                const newsletterName = global.newsletterName || "⏤͟͞ू⃪፝͜⁞⟡ 𝐄llen 𝐉ᴏ𝐄's 𝐒ervice"
                const thumbnailSource = global.icons || global.icon || global.thumb || ''
                const jpegThumbnail = Buffer.isBuffer(thumbnailSource)
                    ? thumbnailSource
                    : thumbnailSource
                        ? Buffer.from(String(thumbnailSource), /^data:image\/.+;base64,/.test(String(thumbnailSource)) ? 'base64' : undefined)
                        : undefined
                const contextInfo = {
                    ...(options.contextInfo || {}),
                    quotedMessage: quoted?.message,
                    participant: quoted?.sender || quoted?.key?.participant || quoted?.key?.remoteJid,
                    stanzaId: quoted?.id || quoted?.key?.id,
                    remoteJid: quoted?.chat || quoted?.key?.remoteJid || jid,
                    isForwarded: true,
                    forwardingScore: 999,
                ...
                return conn.relayMessage(jid, {
                    extendedTextMessage: {
                        ...
                        description: '꒰ 🦈 ᑲіᥱᥒ᥎ᥱᥒіძ᥆ ᥲᥣ ᑲ᥆𝗍 ძᥱ 𝐄llen 𝐉ᴏ𝐄 ꒱',
                        title: "⏤͟͞ू⃪  ̸̷͢𝐄llen͟ 𝐉ᴏ𝐄's͟ 𝐒ervice͟˚₊·—̳͟͞͞♡̥",
                        previewType: 'shadow',
