...
import { installExternalReply, stripUnsafeExternalAdReply } from './externalReply.js'
...
const proto = WAProto || baileys.proto || baileys.default?.WAProto || baileys.default?.proto


const isGroupJid = (jid = '') => String(jid || '').endsWith('@g.us')
const isPrivateUserJid = (jid = '') => {
    jid = String(jid || '')
    return jid.endsWith('@s.whatsapp.net') || jid.endsWith('@lid') || isLidUser?.(jid)
}
const shouldTagAiMessage = (jid, content = {}) => {
    if (!content || typeof content !== 'object' || content.ai !== undefined || isGroupJid(jid) || !isPrivateUserJid(jid)) return false
    if (content.delete || content.react || content.edit || content.forward || content.pin || content.poll) return false
    return Boolean(content.text || content.caption || content.image || content.video || content.audio || content.document || content.sticker || content.contacts || content.location)
}
const withBailsAiTag = (jid, content) => shouldTagAiMessage(jid, content) ? { ...content, ai: true } : content
const sanitizeMessageContent = (content = {}) => stripUnsafeExternalAdReply(content)

export function makeWASocket(connectionOptions, options = {}) {
    /**
     * @type {import('baileys').WASocket | import('baileys').WALegacySocket}
     */
    let conn = (global.opts['legacy'] ? makeWALegacySocket : (_makeWaSocketDefault || baileys.default))(connectionOptions)
    const nativeSendMessage = conn.sendMessage?.bind(conn)
    const nativeRelayMessage = conn.relayMessage?.bind(conn)
    let relayingThroughSendMessage = false
    if (nativeSendMessage) {
        conn.sendMessage = (jid, content = {}, options = {}) => nativeSendMessage(jid, withBailsAiTag(jid, sanitizeMessageContent(content)), { ...options, ...stripUnsafeExternalAdReply(options) })
    }
    if (nativeRelayMessage && nativeSendMessage) {
        conn.relayMessage = async (jid, message = {}, options = {}) => {
            if (relayingThroughSendMessage) return nativeRelayMessage(jid, message, options)
            if (message?.extendedTextMessage && Object.keys(message).length === 1 && !message.extendedTextMessage.contextInfo?.forwardedNewsletterMessageInfo) {
                const { text = '', contextInfo, jpegThumbnail, matchedText, canonicalUrl, title, description } = message.extendedTextMessage
                const externalAdReply = contextInfo?.externalAdReply || (title || description || jpegThumbnail || canonicalUrl ? {
                    title,
                    body: description,
                    thumbnail: jpegThumbnail,
                    sourceUrl: canonicalUrl || matchedText,
                    mediaType: 1,
                    renderLargerThumbnail: false
                } : undefined)
                relayingThroughSendMessage = true
                try {
                    return await conn.sendMessage(jid, {
                        text,
                        linkPreview: null,
                        contextInfo: { ...(contextInfo || {}), ...(externalAdReply ? { externalAdReply } : {}) }
                    }, options)
                } finally {
                    relayingThroughSendMessage = false
                }
            }
            if (message?.liveLocationMessage && Object.keys(message).length === 1) {
                ...
                try {
                    return await conn.sendMessage(jid, {
                        location: {
                            ...
                            name: loc.name || loc.caption
                        },
                        contextInfo: loc.contextInfo
                    }, options)
                } finally {
                    relayingThroughSendMessage = false
                }
...
reply: {
            ...
            async value(jid, text = '', quoted, options = {}) {
                ...
                if (options?.contextInfo?.externalAdReply && quoted) return conn.sendExternalMessage(quoted, text, { contextInfo: stripUnsafeExternalAdReply(options).contextInfo })
                const cleanOptions = stripUnsafeExternalAdReply(options)
                const sent = await conn.sendMessage(jid, { ...cleanOptions, text }, { quoted, ...cleanOptions })
    ...
    async value(buffer) {
        ...
        const resz = jimp_1.getWidth() > jimp_1.getHeight() ? jimp_1.resize({ w: 550 }) : jimp_1.resize({ h: 650 })
...
return conn.sendMessage(jid, { poll: { name, values, selectableCount }})
    ...
    conn.sendMessage(jid, { video: gif, gifPlayback: true, gifAttribution: b, caption: text, footer: footer, jpegThumbnail: file, templateButtons: but, ...options })
        ...
        parseMention: {
             ...
             */
            value(text = '') {
                return [...String(text || '').matchAll(/@([0-9]{5,16}|[0-9A-Za-z._-]+@lid|0)/g)]
                    .map(v => v[1] === '0' ? '0@s.whatsapp.net' : (v[1].endsWith('@lid') ? v[1] : v[1] + '@s.whatsapp.net'))
            },
            enumerable: true
        },
        getName: {
            ...
            value(jid = '', withoutContact = false) {
                ...
                if (jid.endsWith('@g.us')) return new Promise(async (resolve) => {
                    ...
                    conn.user :
                    (conn.chats[jid] || {})
                let userName = global.db.data.users[jid]?.name || global.db.data.users[jid.replace('@s.whatsapp.net', '')]?.name
                if (!userName) {
                    userName = jid.endsWith('@lid') || isLidUser?.(jid)
                        ? (v.name || v.notify || v.verifiedName || jid.split('@')[0])
                        : PhoneNumber('+' + jid.replace('@s.whatsapp.net', '')).getNumber('international')
                }
                const resolvedName = (withoutContact ? '' : v.name) || v.subject || v.vname || v.notify || v.verifiedName || userName
                return resolvedName
            },
            enumerable: true
        },
        loadMessage: {
             ...
             */
            value(messageID, jid = null) {
                return store.loadMessage(jid || messageID, jid ? messageID : null)
            },
            enumerable: true
        },
        sendGroupV4Invite: {
            ...
            async value(m) {
                ...
                for (const message of m) {
                    try {
                        ...
                        if (message.message?.[mtype]?.contextInfo?.quotedMessage) {
                            ...
                            if ((remoteJid && remoteJid !== 'status@broadcast') && quoted) {
                                ...
                                const qM = {
                                    ...
                                    message: fastClone(quoted),
                                    ...(isGroup ? { participant } : {})
                                }
                                // Los mensajes citados se delegan al makeInMemoryStore nativo de Bails.
                            }
                        }
                        ...
                        chats.isChats = true
                        // El historial de mensajes ya lo mantiene store.bind(sock.ev); no duplicarlo en conn.chats.
                    } catch (e) {
                        console.error(e)
                    }
                }
...
export function serialize() {
    ...
    if (!proto?.WebMessageInfo?.prototype) {
        console.warn('WAProto.WebMessageInfo no está disponible; se omite la extensión segura de prototipos.')
        return
    }
    ...
    return (this?.fromMe || areJidsSameUser(this.conn?.user.id, this.sender)) && this.id.startsWith('3EB0') && (this.id.length === 20 || this.id.length === 22 || this.id.length === 12) || false
        ...
        quoted: {
            get() {
                ...
                return Object.defineProperties(fastClone(typeof q === 'string' ? { text: q } : q), {
                    mtype: {
                        get() {
                            return type
                        },
                    ...
                    replyExternal: {
                        /**
                         * Send a safe external reply to the quoted message.
                         * @param {String} text
                         * @param {Object} options
                         */
                        value(text, options = {}) {
                            return self.conn?.sendExternalMessage(this.vM, text, options)
                        },
                        enumerable: true,
                    },
                    copy: {
        ...
        replyExternal: {
            /**
             * Safe external reply based on extendedTextMessage + forwardedNewsletterMessageInfo.
             * Usage inside any command: await m.replyExternal('Mi mensaje')
             * @param {String} text
             * @param {Object} options
             */
            value(text, options = {}) {
                return this.conn?.sendExternalMessage(this, text, options)
            },
            enumerable: true
        },
        copy: {
