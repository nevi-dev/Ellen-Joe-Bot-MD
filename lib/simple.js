        ...
        parseMention: {
            ...
            value(text = '') {
                ...
                return [...String(text || '').matchAll(/@([0-9]{5,16}|[0-9A-Za-z._-]+@lid|0)/g)]
                    .map(v => v[1] === '0' ? '0@s.whatsapp.net' : (v[1].endsWith('@lid') ? adapter?.resolveJid?.(v[1]) || v[1] : v[1] + '@s.whatsapp.net'))
            },
            enumerable: true
        },
        getName: {
            ...
            value(jid = '', withoutContact = false) {
                const adapter = global.db?.adapter
                jid = adapter?.resolveJid?.(jid) || jid
                ...
                if (jid.endsWith('@g.us')) return new Promise(async (resolve) => {
                    ...
                    conn.user :
                    (conn.chats[jid] || {})
                let userName = global.db.data.users[jid]?.name || global.db.data.users[jid.replace('@s.whatsapp.net', '')]?.name || adapter?.getCachedName?.(jid)
                if (!userName) {
                ...
                adapter?.upsertContact?.({ jid, name: resolvedName, notify: v.notify, verifiedName: v.verifiedName })
                return resolvedName
            },
            enumerable: true
        },
                ...
                for (const message of m) {
                    try {
                        ...
                        if (!['protocolMessage'].includes(mtype) && !fromMe && message.messageStubType != WAMessageStubType.CIPHERTEXT && message.message) {
                            ...
                            chats.messages[message.key.id] = fastClone(message)
                            let chatsMessages
