...
import { patchGroupMetadata, resolveJidSync } from '../core/jid-utils.js'
    ...
    let sock = Object.defineProperties(conn, {
        ...
        decodeJid: {
            value(jid) {
                ...
                return resolveJidSync(jid.decodeJid(), conn)
            }
        },
        logger: {
            get() {
...
export function serialize() {
    ...
    return (this?.fromMe || areJidsSameUser(this.conn?.user.id, this.sender)) && this.id.startsWith('3EB0') && (this.id.length === 20 || this.id.length === 22 || this.id.length === 12) || false
        ...
        quoted: {
            get() {
                ...
                return Object.defineProperties(JSON.parse(JSON.stringify(typeof q === 'string' ? { text: q } : q)), {
                    ...
                    sender: {
                        get() {
                            return resolveJidSync((contextInfo.participant || this.chat || '').decodeJid(), self.conn)
                        },
                        enumerable: true
                    },
                    fromMe: {
        ...
        mentionedJid: {
            get() {
                return (this.msg?.contextInfo?.mentionedJid || []).map((jid) => resolveJidSync(jid, this.conn)).filter(Boolean)
            },
            enumerable: true
        },
        name: {
