...
const STORE_PRUNE_INTERVAL = 60 * 60 * 1000
...
function replaceCollectionValues(collection, values) {
  if (!collection) return
  if (typeof collection.clear === 'function') collection.clear()
  else if (typeof collection.flush === 'function') collection.flush()
  else for (const key of Object.keys(collection)) delete collection[key]

  for (const value of values) {
    if (typeof collection.insert === 'function') collection.insert(value)
    else if (typeof collection.upsert === 'function') collection.upsert(value)
    else if (typeof collection.set === 'function') collection.set(value?.key?.id || value?.id, value)
    else if (value?.key?.id || value?.id) collection[value.key?.id || value.id] = value
  }
}

function pruneMessageBucket(bucket) {
  const messages = getCollectionValues(bucket)
  if (messages.length <= MAX_MESSAGES_PER_CHAT) return 0

  const sorted = messages
    .filter(Boolean)
    .sort((a, b) => Number(a?.messageTimestamp || a?.timestamp || 0) - Number(b?.messageTimestamp || b?.timestamp || 0))
  const keep = sorted.slice(-MAX_MESSAGES_PER_CHAT)
  replaceCollectionValues(bucket, keep)
...
function pruneStoreMessages() {
  let pruned = 0
  try {
    const messages = nativeStore?.messages
    if (!messages) return pruned

    if (typeof messages.all === 'function') {
      for (const bucket of messages.all() || []) pruned += pruneMessageBucket(bucket)
    } else {
      for (const bucket of Object.values(messages || {})) pruned += pruneMessageBucket(bucket)
    }

    if (pruned > 0) nativeStore?.writeToFile?.(STORE_FILE)
    if (pruned > 0) console.log(`♻️ Store podado: ${pruned} mensajes antiguos eliminados; máximo ${MAX_MESSAGES_PER_CHAT} por chat.`)
  } catch (error) {
    console.error('No se pudo podar el store de mensajes:', error)
  }
  return pruned
}

const storePruneInterval = setInterval(pruneStoreMessages, STORE_PRUNE_INTERVAL)
storePruneInterval.unref?.()
...
const exportedStore = {
  bind,
  loadMessage,
  reset: pruneStoreMessages,
  prune: pruneStoreMessages,
