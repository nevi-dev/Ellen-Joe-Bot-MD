...
const nativeStore = makeInMemoryStore?.({ logger: pino({ level: 'silent' }) })
...
function installSmartCache(conn) {
  ...
  conn.getSmartGroupMetadata = async function getSmartGroupMetadata(jid, { maxAge = GROUP_METADATA_TTL, force = false } = {}) {
    ...
    if (!force) {
      ...
      if (cached) return cacheGroupLocally(this, cached)
    }
...
async function loadMessage(jid, id = null) {
  ...
  const message = jid ? await nativeStore?.loadMessage?.(jid, id) : null
...
export default {
  ...
  bind,
  loadMessage,
  readFromFile: nativeStore?.readFromFile?.bind(nativeStore),
  writeToFile: nativeStore?.writeToFile?.bind(nativeStore),
  chats: nativeStore?.chats,
  contacts: nativeStore?.contacts,
  messages: nativeStore?.messages,
}
