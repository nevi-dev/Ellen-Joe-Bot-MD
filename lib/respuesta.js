...
const handler = async (type, conn, m, comando) => {
    ...
    try {
        ...
        // Generamos metadatos únicos para que WhatsApp crea que es una imagen real "subida"
        ...
        if (msgText) {
            const content = {
                // Usamos imageMessage "fantasma" para que la miniatura sea nítida y pequeña
                imageMessage: {
                    url: imgUrl,
                    mimetype: 'image/jpeg',
                    caption: msgText, // El texto de Ellen va aquí como leyenda
                    ...
                    // Forzamos el renderizado de miniatura pequeña lateral
                    viewOnce: false
                }
            };
        ...
        console.error('Error en el envío de Ellen:', e);
    }
};
