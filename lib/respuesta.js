...
const handler = async (type, conn, m, comando) => {
    ...
    try {
        // 1. Descargamos la imagen de GitHub
        const response = await fetch(imgUrl);
        const buffer = await response.buffer();

        // 2. Subida real al servidor (Usamos Readable para evitar el error de 'replace')
        const stream = Readable.from(buffer);
        const upload = await conn.waUploadToServer(stream, { 
            mediaType: 'thumbnail-link' 
        });

        // 3. Obtenemos los metadatos oficiales
        ...
        if (msgText) {
            const content = {
                extendedTextMessage: {
                    text: msgText,
                    matchedText: redes,
                    description: "🦈 Ellen JoE - Status: Denied",
                    title: packname,
                    previewType: 0,
        ...
        console.error('Error en Ellen Handler:', e);
    }
};
