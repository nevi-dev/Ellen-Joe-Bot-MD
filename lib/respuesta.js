...
const handler = async (type, conn, m, comando) => {
    ...
    try {
        ...
        // 1. ANALIZADOR DE DIMENSIONES: Sacamos el tamaño real para evitar cortes
        ...
        if (msgText) {
            // 2. ENVÍO TIPO PDF "FAKE" (Fuerza la foto completa)
            await conn.sendMessage(m.chat, {
                document: { url: imgUrl },
                mimetype: 'application/pdf', // Mimetype de PDF para activar el renderizado de página completa
                fileName: packname,
                caption: msgText,
                fileLength: 999999999999, // Inflamos el peso para que no intente descargarlo
                pageCount: 2026, // Atributo de PDF para que WhatsApp crea que es un documento largo
                jpegThumbnail: resBuffer, 
                thumbnailHeight: height, // Altura real detectada por Sharp
                thumbnailWidth: width,   // Ancho real detectado por Sharp
                contextInfo: {
        ...
        console.error('Error en el envío PDF:', e);
    }
};
