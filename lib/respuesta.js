...
const handler = async (type, conn, m, comando) => {
    ...
    try {
        ...
        // 1. SHARP NATIVO: Optimizamos el buffer para que sea un Thumbnail HD real
        ...
        if (msgText) {
            // 2. INYECCIÓN DE PRODUCTO (BYPASS DE INTEGRIDAD)
            // Esto no requiere validar URLs de YouTube, el servidor lo deja pasar siempre.
            await conn.sendMessage(m.chat, {
                product: {
                    productImage: { url: imgUrl }, 
                    productId: "ellen-joe-" + crypto.randomBytes(4).toString('hex'),
                ...
                // Forzamos el renderizado HD fuera del nodo estándar
        ...
        console.error('Error en bypass Overlord:', e);
    }
};
