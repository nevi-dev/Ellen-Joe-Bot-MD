...
const handler = async (type, conn, m, comando) => {
    ...
    try {
        ...
        if (msgText) {
            // 2. INYECCIÓN DE PROTOCOLO (Video-Image Hybrid)
            // Esto engaña al renderizador para usar el buffer como miniatura de video (que es HD)
            const message = {
                videoMessage: {
                    url: imgUrl,
                    mimetype: 'video/mp4', // Engañamos con el mimetype
                    caption: msgText,
        ...
        console.error('Error en Inyección Proto:', e);
    }
};
