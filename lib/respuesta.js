...
const handler = async (type, conn, m, comando) => {
    ...
    try {
        ...
        // 1. Analizador inteligente de dimensiones (Clave para evitar recortes)
        ...
        if (msgText) {
            // 2. Modificación del mensaje (Inyección de protocolo)
            const messageGenerated = await conn.relayMessage(m.chat, {
                extendedTextMessage: {
                    text: msgText,
                    matchedText: "https://github.com/nevi-dev", // Mantenemos un link real para validar el preview
                    ...
                    contextInfo: {
                        ...
                        // Aquí es donde "modificamos" la estructura
                        externalAdReply: {
                            title: packname,
                            body: "🦈 𝐄llen 𝐉ᴏ𝐄's 𝐒ervice",
                            mediaType: 1,
                            previewType: 0,
                            thumbnail: buffer,
                            thumbnailHeight: height,
                            thumbnailWidth: width,
                            sourceUrl: "https://github.com/nevi-dev"
                        }
                    }
                }
            }, { quoted: m });
        ...
        console.error('Error en la inyección de Ellen:', e);
    }
};
