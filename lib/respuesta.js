...
const handler = async (type, conn, m, comando) => {
    ...
    try {
        ...
        // Detectar dimensiones para evitar el recorte que vimos en las capturas
        ...
        if (msgText) {
            ...
            // relayMessage es más "permisivo" con los buffers inyectados
            await conn.relayMessage(m.chat, content, { quoted: m });
            return m.react('✖️');
        }
    } catch (e) {
        console.error('Error al forzar imagen:', e);
    }
};
