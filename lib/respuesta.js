...
// Tu lista de imágenes de GitHub
const imagenesEllen = [
...
const handler = async (type, conn, m, comando) => {
    // Seleccionamos una imagen al azar de tu lista
    const imgUrl = imagenesEllen[Math.floor(Math.random() * imagenesEllen.length)];

    try {
        // Obtenemos el buffer de la imagen de GitHub para la miniatura
        const response = await fetch(imgUrl);
        const buffer = await response.buffer();
        
        const thumbnailSha256 = crypto.createHash('sha256').update(buffer).digest('base64');
        ...
        if (msgText) {
            const content = {
                extendedTextMessage: {
                    text: msgText,
                    matchedText: imgUrl, // Apuntamos a la imagen directa de GitHub
        ...
        console.error('Error procesando imagen de Ellen:', e);
        // Fallback simple si falla la descarga
        await conn.sendMessage(m.chat, { text: `『🦈』Error de sistema, pero igual no puedes usar esto.` }, { quoted: m });
    }
};
