...
export function syncEconomyFromGlobal(users = appDb?.data?.users || {}, { force = false } = {}) { const now = Date.now(); if (!force && now - lastGlobalSync < GLOBAL_SYNC_INTERVAL_MS) return; db.transaction((rows) => rows.forEach(([jid, user]) => syncEconomyUser(jid, user)))(Object.entries(users || {})); lastGlobalSync = now }
...
export function stealCard(thiefId, cardNumber) { return db.transaction(() => { const card = db.prepare("SELECT * FROM cards WHERE card_number=? AND status='active'").get(String(cardNumber)); if (!card || card.user_id === thiefId) throw new Error('Tarjeta inválida'); const total = toSafeInteger(card.balance); if (total <= 0) throw new Error('La tarjeta no tiene fondos'); const thief = appDb.data.users[thiefId] || (appDb.data.users[thiefId] = {}); thief.coin = toSafeInteger(thief.coin) + total; db.prepare('UPDATE cards SET balance=0 WHERE id=?').run(card.id); syncEconomyUser(thiefId, thief); ledger({ userId: card.user_id, type: 'egreso', concept: 'Robo por exposición de tarjeta', amount: total, source: card.card_number, destination: thiefId }); ledger({ userId: thiefId, type: 'ingreso', concept: 'Robo de tarjeta', amount: total, source: card.card_number, destination: 'wallet' }); return total })() }
...
export function runPayroll(referenceDate = new Date()) { return db.transaction(() => { const pay = payrollDate(referenceDate); const payEnd = Math.floor(pay.getTime() / 1000); let count = 0; for (const c of db.prepare("SELECT c.*, j.title,j.salary,j.currency FROM contracts c JOIN jobs j ON j.code=c.job_code WHERE c.status='active'").all()) { const start = Math.max(c.last_paid_until || c.hired_at, c.hired_at); const days = Math.max(0, Math.ceil((payEnd - start) / DAY_MS)); if (!days) continue; const amount = Math.round((c.salary / 15) * Math.min(days, 15)); const user = appDb?.data?.users?.[c.user_id] || {}; if (c.currency === FOREIGN_CURRENCY) user.dennyPremium = toSafeInteger(user.dennyPremium) + amount; else user.bank = toSafeInteger(user.bank) + amount; syncEconomyUser(c.user_id, user); db.prepare('UPDATE contracts SET last_paid_until=? WHERE id=?').run(payEnd, c.id); ledger({ userId: c.user_id, type: 'ingreso', concept: `Nómina ${c.title} (${days} días)`, amount, currency: c.currency, source: 'payroll', destination: c.user_id, createdAt: payEnd }); count++ } return count })() }
export function requestLoan(userId, amount) { return db.transaction(() => { syncEconomyUser(userId, appDb?.data?.users?.[userId] || {}); const u = db.prepare('SELECT * FROM economy_users WHERE jid=?').get(userId); if (db.prepare("SELECT 1 FROM loans WHERE user_id=? AND status='active'").get(userId)) throw new Error('Ya tienes un préstamo activo'); if (u.credit_score < 520) throw new Error('Credit Score insuficiente'); const max = Math.max(500, (u.credit_score - 450) * 20); amount = Math.min(toSafeInteger(amount), max); const weekly = Math.ceil(amount / 6 * 1.08); const due = nowSec() + 7 * 86400; const user = appDb.data.users[userId]; user.bank = toSafeInteger(user.bank) + amount; syncEconomyUser(userId, user); db.prepare('INSERT INTO loans (user_id,principal,outstanding,weekly_payment,interest_rate,late_interest_rate,next_due_at) VALUES (?,?,?,?,0.08,0.04,?)').run(userId, amount, amount, weekly, due); ledger({ userId, type: 'ingreso', concept: 'Desembolso de préstamo', amount, source: 'Hollow Zero Trust', destination: 'bank' }); return { amount, weekly, due } })() }

export function cashOutCard(userId, cardNumber) { return db.transaction(() => { const card = db.prepare("SELECT * FROM cards WHERE card_number=? AND user_id=? AND status='active'").get(String(cardNumber), userId); if (!card) throw new Error('Tarjeta inválida'); const amount = toSafeInteger(card.balance); if (!amount) throw new Error('La tarjeta no tiene fondos'); const user = appDb.data.users[userId] || (appDb.data.users[userId] = {}); user.coin = toSafeInteger(user.coin) + amount; db.prepare('UPDATE cards SET balance=0 WHERE id=?').run(card.id); syncEconomyUser(userId, user); ledger({ userId, type: 'ingreso', concept: 'Retiro de tarjeta a efectivo', amount, source: card.card_number, destination: 'wallet' }); return amount })() }
export function loadCard(userId, cardNumber, amount) { return db.transaction(() => { amount = toSafeInteger(amount); const card = db.prepare("SELECT * FROM cards WHERE card_number=? AND user_id=? AND status='active'").get(String(cardNumber), userId); if (!card) throw new Error('Tarjeta inválida'); const user = appDb.data.users[userId]; if (toSafeInteger(user.bank) < amount) throw new Error('Fondos insuficientes'); user.bank -= amount; db.prepare('UPDATE cards SET balance=balance+? WHERE id=?').run(amount, card.id); syncEconomyUser(userId, user); ledger({ userId, type: 'egreso', concept: 'Recarga de tarjeta', amount, source: 'bank', destination: card.card_number }); return amount })() }
export function repayLoan(userId, amount) { return db.transaction(() => { amount = toSafeInteger(amount); const loan = db.prepare("SELECT * FROM loans WHERE user_id=? AND status='active' ORDER BY created_at LIMIT 1").get(userId); if (!loan) throw new Error('No tienes préstamo activo'); const user = appDb.data.users[userId]; const payment = Math.min(amount, loan.outstanding); if (toSafeInteger(user.bank) < payment) throw new Error('Fondos insuficientes'); user.bank -= payment; const outstanding = loan.outstanding - payment; db.prepare('UPDATE loans SET outstanding=?, status=?, next_due_at=? WHERE id=?').run(outstanding, outstanding <= 0 ? 'paid' : 'active', outstanding <= 0 ? loan.next_due_at : loan.next_due_at + 7 * 86400, loan.id); user.creditScore = toSafeInteger(user.creditScore, 600) + (outstanding <= 0 ? 25 : 5); syncEconomyUser(userId, user); ledger({ userId, type: 'egreso', concept: 'Pago de préstamo', amount: payment, source: 'bank', destination: 'loan' }); return { paid: payment, outstanding } })() }
export function processOverdueLoans(reference = nowSec()) { return db.transaction(() => { let count = 0; for (const loan of db.prepare("SELECT * FROM loans WHERE status='active' AND next_due_at < ?").all(reference)) { const penalty = Math.ceil(loan.outstanding * loan.late_interest_rate); db.prepare('UPDATE loans SET outstanding=outstanding+?, next_due_at=next_due_at+? WHERE id=?').run(penalty, 7 * 86400, loan.id); const user = appDb?.data?.users?.[loan.user_id] || {}; user.creditScore = Math.max(300, toSafeInteger(user.creditScore, 600) - 35); syncEconomyUser(loan.user_id, user); ledger({ userId: loan.user_id, type: 'egreso', concept: 'Interés moratorio de préstamo', amount: penalty, source: 'loan', destination: 'debt' }); count++ } return count })() }
