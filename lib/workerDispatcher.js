...
import { createWorkerGlobalSnapshot } from './workerGlobals.js'
...
queue.push({ payload: { ...payload, id, globals: createWorkerGlobalSnapshot() }, ctx, resolve: value => { clearTimeout(timer); resolve(value) }, reject: error => { clearTimeout(timer); reject(error) } })
pump()
})
}
