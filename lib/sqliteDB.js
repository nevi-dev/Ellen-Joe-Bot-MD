...
export class BetterSQLiteAdapter {
  ...
  upsertContact(contact = {}) {
    try {
      ...
      this.stmts.upsertContact.run({ jid, lid, name: contact.name || contact.pushName || null, notify: contact.notify || null, verified_name: contact.verifiedName || contact.verified_name || null, raw: null, updated_at: Date.now() })
      return jid
    } catch (error) {
  ...
  upsertGroup(metadata = {}) {
    try {
      ...
      const trx = this.db.transaction(() => {
        this.stmts.upsertGroup.run({ jid, subject: metadata.subject || metadata.name || null, owner: normalizeJid(metadata.owner) || null, participants_count: participants.length, raw: null, updated_at: now })
  ...
  cacheMessage(message = {}) {
    try {
      ...
      this.stmts.upsertMessage.run({ id, chat_jid: chat, sender_jid: sender || null, participant_jid: normalizeJid(key.participant) || null, message_type: messageType, text, timestamp: Number(message.messageTimestamp || message.timestamp || Math.floor(Date.now() / 1000)), raw: null, created_at: Date.now() })
