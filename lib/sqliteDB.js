...
export class BetterSQLiteAdapter {
  ...
  _prepareStatements() {
    this.stmts = {
      ...
      writeKv: this.db.prepare(`INSERT INTO kv_store (key, value, updated_at) VALUES (?, ?, ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = excluded.updated_at`),
  ...
  write(data) {
    ...
    this.stmts.writeKv.run(this.key, payload, Date.now())
  }
  ...
  upsertContact(contact = {}) {
    try {
      ...
      this.stmts.upsertContact.run({ jid, lid, name: contact.name || contact.pushName || null, notify: contact.notify || null, verified_name: contact.verifiedName || contact.verified_name || null, raw: JSON.stringify(contact), updated_at: Date.now() })
      return jid
    } catch (error) {
  ...
  upsertGroup(metadata = {}) {
    try {
      ...
      const trx = this.db.transaction(() => {
        this.stmts.upsertGroup.run({ jid, subject: metadata.subject || metadata.name || null, owner: normalizeJid(metadata.owner) || null, participants_count: participants.length, raw: JSON.stringify(metadata), updated_at: now })
        for (const p of participants) {
          const participantJid = normalizeJid(p.jid || p.id)
          if (!participantJid) continue
          this.stmts.upsertParticipant.run({ group_jid: jid, jid: participantJid, lid: normalizeJid(p.lid) || null, admin: p.admin || null, name: p.name || p.notify || null, updated_at: now })
          this.upsertContact({ jid: participantJid, lid: p.lid, name: p.name, notify: p.notify })
        }
      })
      trx()
      return jid
  ...
  cacheMessage(message = {}) {
    try {
      ...
      this.stmts.upsertMessage.run({ id, chat_jid: chat, sender_jid: sender || null, participant_jid: normalizeJid(key.participant) || null, message_type: messageType, text, timestamp: Number(message.messageTimestamp || message.timestamp || Math.floor(Date.now() / 1000)), raw: JSON.stringify(message), created_at: Date.now() })
      if (sender) this.upsertContact({ jid: sender, name: message.pushName || message.name })
      return id
    } catch (error) {
  ...
  ensureGamePvpUser(jid = '', name = '') {
    try {
      ...
      const now = Date.now()
      this.stmts.ensureGamePvpUser.run(normalized, name || null, now, now)
      return normalized
    } catch (error) {
