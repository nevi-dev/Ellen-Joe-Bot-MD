...
export function normalizeLidReferences(m, sender, participantIndex) {
  ...
  if (m && normalized && normalized !== sender) safeSetProperty(m, 'sender', normalized)
  if (m?.quoted?.sender) safeSetProperty(m.quoted, 'sender', normalize(m.quoted.sender))
  if (Array.isArray(m?.mentionedJid)) safeSetProperty(m, 'mentionedJid', m.mentionedJid.map(normalize))
  return normalized
}
